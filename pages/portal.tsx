...
import { BarChart3, FileText, MessageSquare, Settings, Users, Home, LogOut, Upload, Download, Bell, TrendingUp, AlertCircle, Menu, X } from 'lucide-react';
...
const AuthScreen = ({ onLogin }: { onLogin: (user: UserType) => void }) => {
...
const Dashboard = ({ user, onLogout }: { user: UserType; onLogout: () => void }) => {
  ...
  const condominioData = condominiosData[user.condominio as keyof typeof condominiosData];
...
export default function CondoBIPortal() {
  ...
  return (
    <>
      <Head>
        <title>Portal CondoBI - Sistema de Gestão Condominial</title>
        <meta name="description" content="Portal de gestão condominial com IA" />
      </Head>
      <div>
        {!user ? (
